...
class SensoryFingerprint:
    ...
    Subclasses should override vector() to return a flat numpy array.
    """
    ...
    raw_hash: bytes = b""  # SHA-256 of raw data this was extracted from
...
class SpectralFingerprint(SensoryFingerprint):
    ...
    fingerprint_type: str = FingerprintType.SPECTRAL
    wavelength_bins: List[float] = field(default_factory=list)  # nm or Hz bin edges
    energy_values: List[float] = field(default_factory=list)  # intensity per bin
